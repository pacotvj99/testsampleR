...
#' to 'optimal'.
#' @param weight_se_prec A numeric value between 0 and 1 capturing how much to
...
#' @export
testsampler <- function(data, stratifying, N_sample, na.rm="stop", allocation="optimal",
  ...
  if(!(na.rm %in% c("drop", "stop", "impute"))){
    stop("Invalid na.rm argument")
  } else if(na.rm=="drop" & any(is.na(data$stratifying))) {
    message("NAs detected and removed from the data.")
    data <- data[!is.na(data$stratifying),]
  } else if(na.rm=="impute" & any(is.na(data$stratifying))) {
  ...
  } else if(na.rm=="stop" & any(is.na(data$stratifying))) {
  ...
  if(!(allocation %in% c("optimal", "proportional", "constant"))){
    if(allocation!="manual"){
      stop("Invalid allocation argument")
    } else if(!is.numeric(manual_allocation)){
      stop("Allocation type specified as manual, but no specific allocation was entered")
    }
  } else {
    if(!is.numeric(threshold) || threshold>1 || threshold<0){
      stop("Invalid threshold")
    }
    if(!is.numeric(N_sample) | !is.numeric(N_bins_left) | !is.numeric(N_bins_right) | !is.numeric(min_per_bin)){
      stop("Invalid binning specifications")
    } else if(((N_sample%%1)!=0) |((N_bins_left%%1)!=0) | ((N_bins_right%%1)!=0) | ((min_per_bin%%1)!=0)){
      stop("Invalid binning specifications")
    }
  ...
  if(!(allocation %in% c("optimal", "proportional", "constant", "manual"))){
    if(sum(as.numeric(allocation))==N_sample){
      myallocation <- allocation
    }
  } else if(allocation == "constant"){
    ...
    if(!is.numeric(n_positive)){
      pi1 <- ifelse(is.null(pi1), NA, pi1)
      pi0 <- ifelse(is.null(pi0), NA, pi0)
    ...
    } else if(pi1>1 | pi1<0 | is.na(pi1) | pi0>1 | pi0<0 | is.na(pi0) | max(data$stratifying, na.rm=T)<threshold | min(data$stratifying, na.rm=T)>threshold){
      message("Optimal allocation could not be performed: resorting to proportional allocation")
      myallocation <- proportional_allocation(data=data, N_sample=N_sample, strata="strata", min_per_bin=min_per_bin)
    } else{
  ...
  if(any((myallocation<min_per_bin & strata_count>=min_per_bin) | (myallocation<1) | (strata_count < myallocation))){
